...
class WebDataGenerator(DataGenerator):
    ...
    def create_actions_dashboards(self):
        homepage = Action.objects.create(team=self.team, name="NotFlix homepage view")
        ActionStep.objects.create(action=homepage, event="$pageview", url="http://NotFlix.com", url_matching="exact")

        user_signed_up = Action.objects.create(team=self.team, name="NotFlix signed up")
        ActionStep.objects.create(
            action=user_signed_up,
            event="$autocapture",
            url="http://NotFlix.com/1",
            url_matching="contains",
            selector="button",
        )

        user_paid = Action.objects.create(team=self.team, name="NotFlix paid")
        ActionStep.objects.create(
            action=user_paid,
            event="$autocapture",
            url="http://NotFlix.com/2",
            url_matching="contains",
            selector="button",
        )
        ...
        DashboardItem.objects.create(
            team=self.team,
            dashboard=dashboard,
            name="NotFlix signup -> watching movie",
            filters={
                "actions": [
                    {"id": homepage.id, "name": "NotFlix homepage view", "order": 0, "type": TREND_FILTER_TYPE_ACTIONS},
                    {
                        "id": user_signed_up.id,
                        "name": "NotFlix signed up",
                        "order": 1,
                        "type": TREND_FILTER_TYPE_ACTIONS,
                    },
                    {"id": user_paid.id, "name": "NotFlix paid", "order": 2, "type": TREND_FILTER_TYPE_ACTIONS},
                ],
                "insight": "FUNNELS",
            },
        )
    ...
    def populate_person_events(self, person: Person, distinct_id: str, index: int):
        ...
        self.add_event(
            ...
            properties={"$current_url": "http://NotFlix.com", "$browser": browser, "$lib": "web"},
        )
        ...
        if index % 4 == 0:
            self.add_event(
                ...
                distinct_id=distinct_id,
                properties={
                    "$current_url": "http://NotFlix.com/1",
                    "$browser": browser,
                    "$lib": "web",
                    "$event_type": "click",
                },
            ...
            self.add_event(
                event="$pageview",
                distinct_id=distinct_id,
                properties={"$current_url": "http://NotFlix.com/2", "$browser": browser, "$lib": "web",},
            ...
            if index % 5 == 0:
                self.add_event(
                    ...
                    distinct_id=distinct_id,
                    properties={
                        "$current_url": "http://NotFlix.com/2",
                        "$browser": browser,
                        "$lib": "web",
                        "$event_type": "click",
                    },
                ...
                self.add_event(
                    event="$pageview",
                    distinct_id=distinct_id,
                    properties={"$current_url": "http://NotFlix.com/3", "$browser": browser, "$lib": "web",},
                    timestamp=now() - relativedelta(days=start_day) + relativedelta(seconds=60),
                )
