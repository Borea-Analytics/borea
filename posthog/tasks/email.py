...
def _send_weekly_email_report_for_team(team_id: int) -> None:
    ...
    message = EmailMessage(
        ...
        subject=f"Borea weekly report for {period_start.strftime('%b %d, %Y')} to {period_end.strftime('%b %d')}",
        template_name="weekly_report",
        template_context={
            "preheader": f"Your Borea weekly report is ready! Your team had {compact_number(active_users_count)} active users last week! 🎉",
            "team": team.name,
...
def send_invite(invite_id: str) -> None:
    ...
    message = EmailMessage(
        campaign_key=campaign_key,
        subject=f"{invite.created_by.first_name} invited you to join {invite.organization.name} on Borea",
